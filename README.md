...
- **后台扫描**：实时进度、单剧重扫、任务恢复和错误提示
  - 提交立即返回任务 ID，后台独立执行；扫描内部使用受限并发，进度与结果分别读取
  - 每个任务独立保存，已完成结果按需读取，不随新任务重复重写或一直占用内存
  - 单剧重扫直接查询目标和相同 TMDB 的关联记录，避免先遍历整库
...
- **健康度统计**：海报卡片 + 健康度进度条 + 匹配标签
  ...
  - 页面结果按剧集共享元数据并启用压缩；折叠诊断按需展开，长列表分批显示
...
```sh
go test ./...
node --test frontend/src/scan-performance.test.js
cd frontend
